...
def preview():
    try:
        ...
        if not is_local:
            # On Render, use YouTube Music's web player instead
            return jsonify({
                'success': True,
                'streamUrl': f'https://music.youtube.com/watch?v={video_id}',
                'useExternal': True  # Flag to open in new tab instead of playing inline
            })
        
        # Use multiple strategies for local environment
        strategies = [
            # Strategy 1: Android Embed
            {
                'format': 'bestaudio',
        ...
        # If all strategies fail, fallback to external link
        return jsonify({
            'success': True,
            'streamUrl': f'https://music.youtube.com/watch?v={video_id}',
            'useExternal': True
